...
def some_loop_exercise() -> list:
    ...
    return some_list
    # return [index for index in range(13) if index != 6 and index != 7]
...
def alphabet() -> dict:
    ...
    return {x: i for x, i in enumerate(string.ascii_lowercase, start=1)}
...
def simple_sort(data: List[int]) -> List[list]:
    ...
    for k in data:
        ...
        for v in comparison_list:
            if k < v:
                ("Число большее чем", k, "равно:", v)
                list5.append(v)
        # ("Значение ", v)
        # ("Содержит", len(list5), "значение в списке: ", list5)
    ...
    # ("Всего в словаре: ", len(index_sort_dict))
    ...
    # ("remove_double_val_num_list",remove_double_val_num_list)
    # ("Дупликаты: ", double_number_list)
